...
if (isset ($_POST ["usuario"])&& isset($clavemd5)){
	...
	if ($bd) {
		$sql = "SELECT name, lastname, email FROM admin WHERE nombre_usuario='". $_POST["usuario"]."' AND contraseña_usuario='".$_POST["contraseña"]. "' AND estado_usuario=1"; 
		if ($resultado=$bd -> query($sql)){ //ejecuta el Query
			if ($fila=mysqli_fetch_array($resultado, MYSQLI_ASSOC)) {
				...
				header ('location: http://localhost/lycanbox/inicio_admin.php');
			} else {
